...
export default function TalentHitch() {
  ...
  const playReply = async (text) => {
    ...
    try {
      ...
      const res = await base44.functions.invoke('generateSpeech', { text, voice });
  ...
  const handleSend = async (text) => {
    ...
    setLoading(true);
    try {
      const response = await base44.functions.invoke('talentHitchChat', { message: userMsg, history: messages, page: location.pathname });
      const reply = response?.data?.reply || 'Sorry, I did not catch that.';
