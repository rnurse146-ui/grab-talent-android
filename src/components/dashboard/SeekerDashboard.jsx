...
export default function SeekerDashboard({ user, recentBookings, maybeList }) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-8">
      {/* Hero Banner */}
      <div className="relative rounded-3xl overflow-hidden mb-8 bg-gradient-to-br from-purple-900 via-purple-800/60 to-slate-900 border border-purple-500/20 p-8">
        <div className="absolute top-0 right-0 w-64 h-64 bg-purple-500/10 rounded-full blur-3xl" />
        <div className="relative z-10">
          <p className="text-purple-300 font-medium mb-2">Welcome back, {user?.full_name?.split(' ')[0]} 👋</p>
          <h1 className="text-3xl font-bold mb-3">Find Your Perfect Talent</h1>
          <p className="text-slate-400 mb-6 max-w-md">Discover and book world-class performers for your next event. Swipe, save, and book with ease.</p>
          <Link to={createPageUrl('Discover')}>
            <Button className="bg-purple-600 hover:bg-purple-500 h-12 px-6 text-base">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div>
            ...
            <div className="grid sm:grid-cols-3 gap-4">
              <Link to={createPageUrl('Discover')}>
                <motion.div whileHover={{ scale: 1.02 }} className="p-5 rounded-2xl bg-gradient-to-br from-purple-900/50 to-purple-800/30 border border-purple-500/30 cursor-pointer h-full">
              ...
              <Link to={createPageUrl('MaybeList')}>
                <motion.div whileHover={{ scale: 1.02 }} className="p-5 rounded-2xl bg-gradient-to-br from-pink-900/50 to-pink-800/30 border border-pink-500/30 cursor-pointer h-full">
                  <Heart className="w-8 h-8 text-pink-400 mb-3" />
                  <h3 className="font-semibold mb-1">Maybe List</h3>
                  <p className="text-sm text-slate-400">{maybeList.length} saved</p>
                </motion.div>
              </Link>
              <Link to={createPageUrl('Messages')}>
                <motion.div whileHover={{ scale: 1.02 }} className="p-5 rounded-2xl bg-gradient-to-br from-blue-900/50 to-blue-800/30 border border-blue-500/30 cursor-pointer h-full">
                  ...
                  <p className="text-sm text-slate-400">Chat with talent</p>
                </motion.div>
              </Link>
            </div>
          </div>
          ...
          <div className="bg-slate-900/50 rounded-2xl border border-slate-800 p-6">
            ...
            {recentBookings.length === 0 ? (
              <div className="text-center py-10">
                ...
                <Link to={createPageUrl('Discover')}>
                  <Button className="bg-purple-600 hover:bg-purple-500">Find Talent</Button>
                </Link>
              </div>
            ) : (
                ...
                {recentBookings.map((booking) => (
                  <Link key={booking.id} to={createPageUrl('BookingDetails') + `?id=${booking.id}`}>
                    <motion.div whileHover={{ backgroundColor: 'rgba(255,255,255,0.04)' }} className="flex items-center gap-4 p-4 rounded-xl border border-slate-800 cursor-pointer">
                      <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-purple-600/20 to-orange-600/20 flex items-center justify-center shrink-0">
                        <Calendar className="w-4 h-4 text-purple-400" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <h4 className="font-medium truncate">{booking.event_name || 'Event'}</h4>
                        <p className="text-sm text-slate-400">{booking.talent_stage_name} • {booking.event_date}</p>
                      </div>
                      <Badge className={
          ...
          {maybeList.length > 0 && (
            <div className="bg-slate-900/50 rounded-2xl border border-slate-800 p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <Link to={createPageUrl('MaybeList')}>
                  <Button variant="ghost" size="sm" className="text-slate-400 text-xs">View all</Button>
                </Link>
              </div>
              <div className="space-y-3">
                {maybeList.slice(0, 4).map((item) => (
                  <Link key={item.id} to={createPageUrl('TalentProfile') + `?id=${item.talent_profile_id}`}>
                    <div className="flex items-center gap-3 hover:bg-slate-800/50 rounded-lg p-1 transition-colors">
                      <div className="w-10 h-10 rounded-full bg-slate-800 overflow-hidden shrink-0">
                        {item.talent_photo ? <img src={item.talent_photo} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center"><Users className="w-4 h-4 text-slate-600" /></div>}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="font-medium text-sm truncate">{item.talent_stage_name}</p>
                        <p className="text-xs text-slate-400 capitalize">{item.talent_category?.replace(/_/g, ' ')}</p>
                      </div>
                    </div>
                  </Link>
                ))}
