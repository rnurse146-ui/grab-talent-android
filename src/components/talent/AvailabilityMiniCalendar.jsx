...
export default function AvailabilityMiniCalendar({ talentProfileId }) {
    ...
    <div className="bg-slate-900/50 rounded-2xl border border-slate-800 overflow-hidden">
      ...
      <div className="grid grid-cols-7 gap-px px-2 pb-3">
        {days().map(day => {
          ...
          let cls = 'relative flex items-center justify-center aspect-square rounded-lg text-xs font-medium transition-colors ';
          if (!inMonth || past) cls += 'opacity-20 text-slate-600 ';
          else if (isBooked) cls += 'bg-purple-600/30 text-purple-300 ';
          else if (isBlocked) cls += 'bg-red-500/20 text-red-400 ';
          else cls += 'text-slate-300 ';
          if (isToday(day)) cls += 'ring-1 ring-purple-500 ';

          return (
            <div key={dateStr} className={cls} title={isBooked ? 'Booked' : isBlocked ? 'Unavailable' : 'Available'}>
              {format(day, 'd')}
